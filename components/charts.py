...
def trend_chart(df_paid: pd.DataFrame):
    ...
    # konversi USD ke IDR untuk chart
    df["idr_total"] = df.apply(lambda r: r["total_akhir"]*r["fx_rate"] if r["currency"]=="USD" else r["total_akhir"], axis=1)
...
def donut_divisi(df_paid: pd.DataFrame):
    ...
    if df_exp.empty:
        ...
        return
    df_exp = df_exp.copy()
    df_exp["idr_total"] = df_exp.apply(lambda r: r["total_akhir"]*r["fx_rate"] if r["currency"]=="USD" else r["total_akhir"], axis=1)
...
def budget_variance_chart(budget_df: pd.DataFrame, actual_df: pd.DataFrame):
    # budget_df: bulan, divisi, kategori, planned
    # actual_df: transaksi Paid agregasi per divisi+kategori+bulan
    if budget_df.empty:
        st.info("Belum ada data anggaran.")
        return
    # siapkan actual agregasi
    if not actual_df.empty:
        actual_df = actual_df.copy()
        actual_df["bulan"] = pd.to_datetime(actual_df["tanggal"]).dt.strftime("%Y-%m")
        actual_df["idr"] = actual_df.apply(lambda r: r["total_akhir"]*r["fx_rate"] if r["currency"]=="USD" else r["total_akhir"], axis=1)
