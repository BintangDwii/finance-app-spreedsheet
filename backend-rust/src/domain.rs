...
#[sqlx(type_name = "role_t", rename_all = "PascalCase")]
...
pub enum TransaksiStatus {
    #[sqlx(rename = "Pending (Butuh Approval)")]
    #[serde(rename = "Pending (Butuh Approval)")]
    Pending,
    #[sqlx(rename = "Approved (Menunggu Bayar)")]
    #[serde(rename = "Approved (Menunggu Bayar)")]
    Approved,
    #[sqlx(rename = "Paid (Lunas)")]
    #[serde(rename = "Paid (Lunas)")]
    Paid,
    #[sqlx(rename = "Rejected (Ditolak)")]
    #[serde(rename = "Rejected (Ditolak)")]
    Rejected,
    #[sqlx(rename = "Reconciled")]
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, sqlx::Type)]
#[sqlx(type_name = "jenis_t", rename_all = "PascalCase")]
pub enum Jenis {
    Pemasukan,
    Pengeluaran,
    #[sqlx(rename = "Transfer Internal")]
...
mod tests {
    ...
    fn newtype_ids_distinct() {
        ...
        // Compile-time distinctness — types not interchangeable (try to assign uid to tid fails at compile)
        assert_eq!(uid.0, 1);
        assert_eq!(tid.0, 1);
        // Hash
