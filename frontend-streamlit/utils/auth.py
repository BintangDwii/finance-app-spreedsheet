...
# NOTE: Direct DB auth removed — canonical login is via api_client.login (Axum JWT).
# hash_pw/verify_user kept as deprecated stubs for backwards compat (not used).

def hash_pw(pw: str) -> str:  # deprecated
    import hashlib
    return hashlib.sha256(pw.encode()).hexdigest()

def verify_user(username: str, password: str):  # deprecated — use api_client.login
    return None
